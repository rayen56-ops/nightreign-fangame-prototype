...
func _move_camera_to_player() -> void:
	...
	position = Vector2(
		float(target.grid_pos.x * Constants.TILE_SIZE) + WorldTiles.tile_size / 2.0 - hud_offset + inventory_width / 2.0,
		float(target.grid_pos.y * Constants.TILE_SIZE) + WorldTiles.tile_size / 2.0
	)
...
func _on_world_initialized() -> void:
	...
	if World.player:
		...
		if target:
			position = Vector2(
				float(target.grid_pos.x * Constants.TILE_SIZE) + WorldTiles.tile_size / 1.0,
				float(target.grid_pos.y * Constants.TILE_SIZE) + WorldTiles.tile_size / 2.0
			)
