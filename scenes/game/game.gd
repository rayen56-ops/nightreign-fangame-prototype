...
func _ready() -> void:
	...
	_initialize()
	add_child(preload("res://src/nightreign/expedition/night_overlay.gd").new())
	...
	Modals.inventory_opened.connect(
		func(inventory: InventoryModal) -> void:
			...
			inventory.message_logged.connect(
				func(message: String) -> void: World.message_logged.emit(message, LogMessages.Level.NORMAL)
			)
	)
...
func _check_player_input(event: InputEvent) -> BaseAction:
	for kind: String in ["skill", "ultimate"]:
		if event.is_action_pressed(kind):
			...
			var target_direction := await Modals.prompt_for_direction()
