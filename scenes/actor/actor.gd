...
func _ready() -> void:
	...
	if monster == World.player:
		nightreign_visual = preload("res://src/nightreign/characters/wylder_visual.gd").new()
...
func _process(delta: float) -> void:
	...
	if animation_time >= ANIMATION_SPEED and nightreign_visual == null:
		animation_time = 0
	...
	if is_moving:
		if move_time > 0:
			...
			position = move_start_pos.lerp(move_target_pos, clampf(t, 0.0, 1.0)).round()
...
func set_facing_direction(direction: Vector2i) -> void:
	if nightreign_visual:
		nightreign_visual.face(direction)
		return
...
func _update_appearance() -> void:
	...
	if monster == World.player and monster.is_dead and nightreign_visual == null:
		# Show a tombstone
